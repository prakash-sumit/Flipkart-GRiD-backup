...
ros::Publisher yaw_motor ("/yaw_motor_to_controls", &yaw_motor_msg); 
...
if(counter_servo ==1 && counter_y ==0 && counter_x ==0) {
    yaw_motor_msg.data = 30150; 
    yaw_motor.publish(&suck_drop);          // 30 to 150
    counter_servo = 100;
    counter_z =1;
}
...
if(counter_servo ==1 && counter_y ==0 && counter_x ==0) {
        yaw_motor_msg.data = 30150; 
        yaw_motor.publish(&yaw_motor_msg);          // 30 to 150
    counter_servo = 100;
    counter_z =1;
}
...
if(raw_count == 1) {
 yaw_motor_msg.data = 15030; 
yaw_motor.publish(&yaw_motor_msg);          //  150 to 30
  delay(5);
  raw_count = 5;
  }   
