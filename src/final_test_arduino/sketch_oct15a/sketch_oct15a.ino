...
else if (task_number==4) {  //backward to tray 
...
// {
//   if(steps_z > tray_z) {  
//    for(int i =0; i<= steps_z - tray_z ; i++) {          
//  move_anticlockwise(stepPinM3,dirPinM3) ;
//  delay(10);
//   }
//   motor_status[2]=2;
...
//   else if (steps_z < tray_z) {       // reason to use while loop is so that hiegth kam ho pehle so no collision
//   for(int i =0; i<= tray_z - steps_z ; i++) {          
//  move_clockwise(stepPinM3,dirPinM3) ;
//  delay(10);
//   }
//   motor_status[2]=2;
...
{
  if(steps_y > tray_theta) {  
   for(int i =0; i<= steps_y - tray_theta ; i++) {          
 move_anticlockwise(stepPinM2,dirPinM2) ;
 delay(30);
  }
  motor_status[1]=2;
  ...
  else if (steps_y < tray_theta) {       // reason to use while loop is so that hiegth kam ho pehle so no collision
  for(int i =0; i<= tray_theta - steps_y ; i++) {          
 move_clockwise(stepPinM2,dirPinM2) ;
 delay(30);
...
{
  if(steps_x > tray_r) {  
   for(int i =0; i<= steps_x - tray_r ; i++) {          
 move_anticlockwise(stepPinM1,dirPinM1) ;
 delay(10);
  }
  motor_status[0]=2;
  ...
  else if (steps_x < tray_r ){      
  for(int i =0; i<= tray_r - steps_x ; i++) {          
 move_clockwise(stepPinM1,dirPinM1) ;
 delay(10) ;
 }
  motor_status[0]=2;
...
if(motor_status[0] == 2 && motor_status[1] == 2 && motor_status[2] == 2 ) {
  str_msg.data =2;    
  task_status.publish(&str_msg);     //2:reached tray
}
