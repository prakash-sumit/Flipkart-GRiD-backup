...
//sucktion 2 mechanim
    myservo1.attach(7);
    myservo1.write(130);                 //degree 30 is the downward position as per night of oct23 
    nh.initNode();
    nh.subscribe(angles);
...
if(counter_y==1) {                  // clockwise is anticlockwise in Y (theta motor)
...
if(counter_y == 0 && counter_x == 0) {
        //servo back to 0
        for(int j=130 ; j >= 35; j--) {
        myservo1.write (j);
        delay(5);
        }
counter_z =1;
...
if ( counter_x == 6 && counter_y == 6 && counter_z == 6) {    
...
if(counter_z ==7 && counter_x == 7 && counter_y ==7) {
    // deactivate suction
...
if (counter_z==8 ) {
    ...
    delay(2); 
     ...
     counter_z = 9;
     counter_y= 8;                    //15 means reached initial head top
     delay(100) ;
     }
}
...
if (counter_x == 9 && counter_y ==9) {

  // servo back to 90 degree 
      for(int j=35 ; j <=130 ; j++) {
        myservo1.write (j);
        delay(5);
        }
    confirmation_msg.data = 1;
