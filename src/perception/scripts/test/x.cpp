...
void tasksCallback (const std_msgs::String& tasklist) {
  char task_name [] = tasklist.data;
}

ros::Subscriber<std_msgs::String> tasklib("/task", tasksCallback);  //to take info from dm about which task to perform
...
else if (task_name == "Backward_to_tray") {  
  
   if(motor_status[0] == 2 && motor_status[1] == 2 && motor_status[2] == 2 ){
    str_msg.data ="Reached_tray" ;
    task_status.publish(&str_msg);
   }
...
else if (task_name == "Backward_to_final") {
...
// else if (task_name == "backward_to_tray") {    //assumed headtop to be (50,50,50)
  ...
  if(motor_status[0] == 3 && motor_status[1] == 3 && motor_status[2] == 3 ) {
    str_msg.data ="Reached_trolley_top";
  task_status.publish(&str_msg);
  }
