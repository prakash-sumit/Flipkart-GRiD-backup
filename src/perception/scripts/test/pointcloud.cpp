...
int main(int argc, char **argv)
{
  ...
  //OpenCV HighGUI call to create a display window on start-up.
    ...
    ros::Subscriber dep;
     dep = nh.subscribe ("/zed2i/zed_node/point_cloud/cloud_registered", 1, depthcallback);
   
   //image_transport::Subscriber depth = it.subscribe("/camera/depth/points",1,depthcallback);
    image_transport::Subscriber sub = it.subscribe("/zed2i/zed_node/rgb/image_rect_color", 1, blobDetectionCallback);
